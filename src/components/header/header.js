...
function Header(){
    ...
    return (
        ...
        <div class="navbar">
            ...
            <span onClick={Togglevisibility} class="navbar__toggle-bar">&#9776; Menu</span>
            <input onChange={(e)=>setsearch(e.target.value)} placeholder="Search" class="navbar__searchbar"/>
            <span class="navbar__signin text-medium">Sign In</span>
        </div>
        <div class={`searchResult ${result?(result.length >0?'active':''):''}`}>
            <div class="close" onClick={()=>(setresult([]))}>X</div>
            {result?(result.length >0 ? result.map((result)=>(
                <>
                <div class="search__box" >
                    <box>
                <img alt="" src={IMG_URL+result.poster_path}/>
                <span class="search_title">{result.original_title}<br/>RELEASE : {result.release_date}<span class="star">★ {result.vote_average}</span></span>
                </box>
